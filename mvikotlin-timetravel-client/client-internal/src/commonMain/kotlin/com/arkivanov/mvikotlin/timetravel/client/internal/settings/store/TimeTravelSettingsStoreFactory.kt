...
internal class TimeTravelSettingsStoreFactory(
    ...
    private val editingConfig: SettingsConfig.Editing
) {
    ...
    fun create(): TimeTravelSettingsStore =
        object : TimeTravelSettingsStore, Store<Intent, State, Nothing> by storeFactory.create(
            ...
            reducer = ReducerImpl(editingConfig = editingConfig)
        ) {}
    ...
    private inner class ExecutorImpl : ReaktiveExecutor<Intent, Nothing, State, Result, Nothing>() {
        ...
        private fun State.Settings.applyEditing(editing: State.Editing): State.Settings? {
            return State.Settings(
                host = editing.host,
                port = editing.port.toIntOrNull() ?: return null,
                connectViaAdb = editing.connectViaAdb,
                wrapEventDetails = editing.wrapEventDetails,
                isDarkMode = editing.isDarkMode ?: isDarkMode
            )
        }
    ...
    private class ReducerImpl(
        ...
        private fun State.Settings.toEditing(): State.Editing =
            State.Editing(
                ...
                connectViaAdb = connectViaAdb,
                wrapEventDetails = wrapEventDetails,
                isDarkMode = isDarkMode.takeIf { editingConfig.isDarkModeEnabled }
            )
    }
